...
function MovieSidebar() {
  ...
  return (
    <div className="fixed top-0 left-0 w-[226px] h-screen flex flex-col items-center justify-start py-4 pb-8 rounded-r-[30px] rounded-b-[30px] border-r-solid border-r-[1px] border-r-white2-500 ">
      <Link
        href="/"
        className="w-full min-h-[90px] flex items-center gap-6 px-4"
      >
        <Image
          src={logoImg}
      ...
      <div className="w-full h-full mt-9 flex flex-col items-center justify-start">
        {sidebarItems.map((d) => (
          <button
            className={twMerge(
              "w-full py-5  hover:bg-red-100 flex items-center justify-start gap-5 px-4 border-r-solid border-r-[4px] text-dark-100 hover:text-red-306 border-r-transparent ",
      ...
      <div className="flex flex-col items-center p-4 py-6 gap-4 w-[170px] rounded-xl border border-pink-400 bg-pink-100 bg-opacity-40">
      ...
      <div className="flex w-[124px] h-[30px] gap-4 my-6">
        <Image
          src="/Logout.png"
          className={"w-[25px]"}
          width={30}
